...
#################################### Yesterday Report #############################################
ReportDateCode = "20220821_"  ## change date before run
...
Suppliers_Rate_Teams = ['#Tylus', '#AEREMIAH', '#JasimDelivery', '#CLC', '#CLC2','#Parcel',"#SWAT","#SWAT_TYLOS","#SWAT_AEREMIAH","#SWAT_JASIM","#SWAT_CLC"]
Parcel_Rate_Teams = []
...
apiaryDf = df
df = df.rename(columns={'deliveryTaskID': 'Task ID', 'date': 'Date', 'distance': 'Distance', 'cashCollected': 'Cash Collected', 'driver': 'Driver ID', 'driverName': 'Driver Name', 'driverFees': 'Driver Fees'})
...
for team in Teams:
    ...
    filtered_column_df = filtered_df.filter(items = ["Task ID","Date","Distance","Cash Collected","Driver ID","Driver Name","Driver Fees"])    # Filtering needed cols
...
######################### Write Raw Data HTML File ######################
    ...
    #####################################################################
    grouped_filtered_col_df = filtered_column_df.groupby(['Driver ID', 'Driver Name'])['Cash Collected','Driver Fees'].sum().reset_index()
 ...
 #################################   # >= 5250 & < 10500 Cat (3) ###################################################
    ...
    if team in Parcel_Rate_Teams:
        column_names = ['Driver ID','Driver Name','Total Orders','Cash Collected','Driver Fees','Supplier Commission','0.700','0.800','0.900','1.100','Extra Distance','Distance Pay', 'Commissions','Net Payable']
    else:
        column_names = ['Driver ID', 'Driver Name', 'Total Orders', 'Cash Collected','Driver Fees','Supplier Commission', '0.800', '0.900', '1.000', '1.200','Extra Distance','Distance Pay', 'Commissions','Net Payable']
   ...
   # result4['cashCollected'] = result4['cashCollected'].apply(lambda x: "BHD {:.2f}".format(x))
    ...
    if str(team) != '#Parcel':
        result4.drop(['0.800','0.900','1.000','1.200','Extra Distance','Distance Pay','Commissions'], axis=1, inplace=True)
    if str(team) == '#Parcel':
