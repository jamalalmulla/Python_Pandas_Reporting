...
######################### Write Raw Data HTML File ######################
    ...
    #     Rate = '0.700'
    rslt_dfLessThan2KM = filtered_df_copy.loc[
        (filtered_df_copy["Distance"] >= 0) & (filtered_df_copy["Distance"] < 1000000)]
 ...
 #    grouped_by_driver_sum_df = grouped_by_driver_sum.reset_index()
    ...
    result4 = pd.merge(result0, driver_occurLessThan2Km_df, how="left", on=["Driver ID"])
    ...
    result4["Commissions"] = result4['0.500'] * Rate1
    #### Calc net payable
    ...
    column_names = ['Driver ID','Driver Name','Total Orders','Cash Collected','0.500','Extra Distance','Distance Pay', 'Commissions','Net Payable']
   ...
   # result4['cashCollected'] = result4['cashCollected'].apply(lambda x: "BHD {:.2f}".format(x))
    ...
    result4.drop(['Extra Distance', 'Distance Pay'], axis=1, inplace=True)
...
##### add bulkOrder and driverFee 0.2
apiaryDf.loc[apiaryDf['team'] == 18, 'bulkOrder'] = True
apiaryDf.loc[apiaryDf['team'] != 18, 'bulkOrder'] = False
apiaryDf.loc[apiaryDf['bulkOrder'] == True, 'driverFees'] = 0.2
