...
while n == 0:
    ...
    DateTo = input("Please enter a valid report 'From'' date 'dd Mmm yyyy': ")
...
Teams = ['#Parcel','#Tylus', '#AEREMIAH', '#JasimDelivery', '#CLC', '#CLC2',"#SWAT","#SWAT_TYLOS","#SWAT_AEREMIAH","#SWAT_JASIM","#SWAT_CLC"]
Suppliers_Rate_Teams = ['#Tylus', '#AEREMIAH', '#JasimDelivery', '#CLC','#Parcel', '#CLC2',"#SWAT","#SWAT_TYLOS","#SWAT_AEREMIAH","#SWAT_JASIM","#SWAT_CLC"]
Parcel_Rate_Teams = []
######################## Define Teams Codes #############################
tylosCode = 998
aeremiahCode = 997
...
apiaryDf = df
df = df.rename(columns={'pickupTaskID': 'Task ID', 'date': 'Date', 'distance': 'Distance', 'cashCollected': 'Cash Collected', 'driver': 'Driver ID', 'driverName': 'Driver Name'})
####################### TeamDesk Import Sheet ###############################################################
teamDeskDf = df.filter(items = ["Task ID","Date","FormUser","Driver ID","Driver Name","Distance","Cash Collected","description","Pick_up_From","fees"])

teamDeskDf['description'] = teamDeskDf['description'].astype(str).str[:100]

#book = load_workbook('initial.xlsx')
writer = pd.ExcelWriter('{ReportDate}Teamdesk_import.xlsx'.format(ReportDate=ReportDateCode), engine='openpyxl')
#writer.book = book
...
for team in Teams:
    ...
    filtered_column_df = filtered_df.filter(items = ["Task ID","Date","Distance","Cash Collected","Driver ID","Driver Name"])    # Filtering needed cols
...
# Writing the Raw Data Excel Sheet
    ...
    # wb = excel.Workbooks.Open(r'C:\Users\Jamal\PycharmProjects\pythonProject\\' + ReportDate + str(team) + '.xlsx')
...
######################### Write Raw Data HTML File ######################
    ...
    with open(team + '_Raw_Data.html', 'w') as f:
        f.write(html_string_Raw_Data.format(table=filtered_column_df.to_html(classes='mystyle', index=False), team = team.replace('#', '%23')))

    #####################################################################

    grouped_filtered_col_df = filtered_column_df.groupby(['Driver ID', 'Driver Name'])['Cash Collected'].sum().reset_index()
    ...
    driver_occur_total_df.rename(columns={'Driver ID': 'Total Orders', 'index': 'Driver ID'}, inplace=True)  # Rename Columns
    Rate = '0.800'
 ...
 #################################   # >= 5250 & < 10500 Cat (3) ###################################################
    ...
    result4 = pd.merge(result3, grouped_by_driver_sum_df[['Driver ID', 'Km > 20', 'Kms > 20 commission']], on='Driver ID',
                       how='left')
    ...
    if team in Parcel_Rate_Teams:
        ...
        Rate3 = 0.9
        Rate4 = 1.1
        result4["Commissions"] = result4['0.700'] * Rate1 + result4['0.800'] * Rate2 + result4['0.900'] * Rate3 + result4['1.100'] * Rate4
    else:
    ...
    if team == '#Tylus':
        ...
        Tylos_commissions = result4['Commissions'].iloc[-1]
    elif team == '#AEREMIAH':
        Aeremiah_net_payable = result4['Net Payable'].iloc[-1]
        Aeremiah_commissions = result4['Commissions'].iloc[-1]
    elif team == '#JasimDelivery':
        Jasim_net_payable = result4['Net Payable'].iloc[-1]
        Jasim_commissions = result4['Commissions'].iloc[-1]
    elif team == '#Parcel':
        Parcel_commissions = result4['Commissions'].iloc[-1]
        Parcel_net_payable = result4['Net Payable'].iloc[-1]
    elif team == '#CLC':
        CLC_commissions = result4['Commissions'].iloc[-1]
        CLC_net_payable = result4['Net Payable'].iloc[-1]
    elif team == '#CLC2':
        CLC2_commissions = result4['Commissions'].iloc[-1]
        CLC2_net_payable = result4['Net Payable'].iloc[-1]
    elif team == '#SWAT':
        SWAT_commissions = result4['Commissions'].iloc[-1]
        SWAT_net_payable = result4['Net Payable'].iloc[-1]
    elif team == '#SWAT_TYLOS':
        SWAT_tylos_commission = result4['Commissions'].iloc[-1]
        SWAT_tylos_net_payable = result4['Net Payable'].iloc[-1]
    elif team == '#SWAT_AEREMIAH':
        SWAT_aeremiah_commission = result4['Commissions'].iloc[-1]
        SWAT_aeremiah_net_payable = result4['Net Payable'].iloc[-1]
    elif team == '#SWAT_JASIM':
        SWAT_jasim_commission = result4['Commissions'].iloc[-1]
        SWAT_jasim_net_payable = result4['Net Payable'].iloc[-1]
    elif team == '#SWAT_CLC':
        SWAT_CLC_commission = result4['Commissions'].iloc[-1]
    ...
    if team in Parcel_Rate_Teams:
        column_names = ['Driver ID','Driver Name','Total Orders','Cash Collected','0.700','0.800','0.900','1.100','Extra Distance','Distance Pay', 'Commissions','Net Payable']
    else:
        column_names = ['Driver ID', 'Driver Name', 'Total Orders', 'Cash Collected', '0.800', '0.900', '1.000', '1.200','Extra Distance','Distance Pay', 'Commissions','Net Payable']

    result4 = result4.reindex(columns=column_names)
    if str(team) == '#Parcel':
        result4.drop(['Total Orders', '0.800', '0.900', '1.000', '1.200', 'Extra Distance', 'Distance Pay', 'Commissions', 'Net Payable'], axis=1, inplace=True)
   ...
   # result4['cashCollected'] = result4['cashCollected'].apply(lambda x: "BHD {:.2f}".format(x))
    ...
    teamNameDateDf = pd.DataFrame({'Team': [team],'Date': ReportDate})
    ...
    if team == '#Parcel':
        ...
        refString = "B5:M{maxRow}".format(maxRow=maxRow+4)
...
############################ HTML Table Visualization #################################################################
   # print(result4.to_html(classes='table table-striped text-center', justify='center'))
    ...
    with open(team + '_Drivers.html', 'w') as f:
        f.write(html_string.format(table=teamNameDateDf.to_html(classes='mystyle', index=False), team = team.replace('#', '%23')))
...
df_temp = pd.DataFrame({'Driver ID': parcelCode, 'Net Payable': Parcel_net_payable, 'Commissions': Parcel_commissions}, index=[0])
df_cash_grouped = pd.concat([df_cash_grouped, df_temp], ignore_index=True, axis=0)
df_temp = pd.DataFrame({'Driver ID': clcCode, 'Net Payable': CLC_net_payable, 'Commissions': CLC_commissions}, index=[0])
df_cash_grouped = pd.concat([df_cash_grouped, df_temp], ignore_index=True, axis=0)
df_temp = pd.DataFrame({'Driver ID': clc2Code, 'Net Payable': CLC2_net_payable, 'Commissions': CLC2_commissions}, index=[0])
df_cash_grouped = pd.concat([df_cash_grouped, df_temp], ignore_index=True, axis=0)
df_temp = pd.DataFrame({'Driver ID': swatCode, 'Net Payable': SWAT_net_payable, 'Commissions': SWAT_commissions}, index=[0])
df_cash_grouped = pd.concat([df_cash_grouped, df_temp], ignore_index=True, axis=0)
df_temp = pd.DataFrame({'Driver ID': tylosCode, 'Net Payable': Tylos_net_payable, 'Commissions': Tylos_commissions}, index=[0])
df_cash_grouped = pd.concat([df_cash_grouped, df_temp], ignore_index=True, axis=0)
df_temp = pd.DataFrame({'Driver ID': aeremiahCode, 'Net Payable': Aeremiah_net_payable, 'Commissions': Aeremiah_commissions}, index=[0])
df_cash_grouped = pd.concat([df_cash_grouped, df_temp], ignore_index=True, axis=0)
df_temp = pd.DataFrame({'Driver ID': jasimCode, 'Net Payable': Jasim_net_payable, 'Commissions': Jasim_commissions}, index=[0])
df_cash_grouped = pd.concat([df_cash_grouped, df_temp], ignore_index=True, axis=0)

df_temp = pd.DataFrame({'Driver ID': swatTylosCode, 'Net Payable': SWAT_tylos_net_payable, 'Commissions': SWAT_tylos_commission}, index=[0])
df_cash_grouped = pd.concat([df_cash_grouped, df_temp], ignore_index=True, axis=0)
df_temp = pd.DataFrame({'Driver ID': swatAremiahCode, 'Net Payable': SWAT_aeremiah_net_payable, 'Commissions': SWAT_aeremiah_commission}, index=[0])
df_cash_grouped = pd.concat([df_cash_grouped, df_temp], ignore_index=True, axis=0)
df_temp = pd.DataFrame({'Driver ID': swatJasimCode, 'Net Payable': SWAT_jasim_net_payable, 'Commissions': SWAT_jasim_commission}, index=[0])
df_cash_grouped = pd.concat([df_cash_grouped, df_temp], ignore_index=True, axis=0)
df_temp = pd.DataFrame({'Driver ID': swatClcCode, 'Net Payable': SWAT_CLC_net_payable, 'Commissions': SWAT_CLC_commission}, index=[0])
df_cash_grouped = pd.concat([df_cash_grouped, df_temp], ignore_index=True, axis=0)
...
#book = load_workbook('initial.xlsx')
writer = pd.ExcelWriter('{ReportDate}Cash.xlsx'.format(ReportDate=ReportDateCode), engine='openpyxl')
#writer.book = book
...
wb = load_workbook('{ReportDate}Cash.xlsx'.format(ReportDate=ReportDateCode))
ws = wb["cash-" + d]
...
wb.save('{ReportDate}Cash.xlsx'.format(ReportDate=ReportDateCode))
wb.close()
...
filepath = "./{ReportDate}Net.xlsx".format(ReportDate=ReportDateCode)
wb = Workbook()
wb.save(filepath)
ws = wb['Sheet']
...
ws['J20'].number_format = '0.00%'
ws['K20'].number_format = '0.00%'
...
wb.save('{ReportDate}Net.xlsx'.format(ReportDate=ReportDateCode))
wb.close()
...
wb = excel.Workbooks.Open(winPath)
# wb = excel.Workbooks.Open(r'C:\Users\Jamal\PycharmProjects\pythonProject\{ReportDate}Net.xlsx'.format(ReportDate=ReportDateCode))
...
apiaryDf.to_csv('./{ReportDate}Apiary_import.csv'.format(ReportDate=ReportDateCode), index=False)
